...
voting2.command('rating', async ctx => {
    ...
    try {
        await ctx.reply([totalBlock, topAnimesBlock, topWeightedBlock, perspectiveAnimesBlock, topWatchedBlock].join('\n\n'))
    } catch (e) {
...
voting2.command('raw', async ctx => {
    ...
    try {
        // TODO: Fix typings
        await ctx.replyWithDocument(new InputFile(Buffer.from(text) as any, 'voting.json'))
    } catch (e) {
        await ctx.reply(`Error: ${e}`)
    }
})
